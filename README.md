...
Пріоритетно визначається по підняттю верхньої губи. Очі або брови можуть бути напружені, але без сильного сигналу верхньої губи ця емоція не повинна спрацьовувати.

| Режим | Умова |
|---|---|
| Абсолютний | `upper_lip_raise < 0.04` та `MAR < 0.15` та `mouth_asymmetry < 0.009` |
| Дельта | `d_lip_raise < -0.02` та `d_mar < 0.08` та `d_asym < 0.005` |

### 5. Angry (Злість)

Визначається по насуплених бровах і/або напружених очах, але розслаблені брови блокують цю категорію, щоб `Sad` не поглинався `Angry`. Сильне підняття верхньої губи також блокує цю категорію, щоб `Angry` не поглинав `Disgusted`.

| Режим | Умова |
|---|---|
| Абсолютний | `smile < 0.005` та `upper_lip_raise >= 0.05` та ((`brow_dist < 0.118` та `EAR < 0.30`) або (`EAR < 0.245` та `MAR < 0.15` та `brow_dist < 0.128`)) |
| Дельта | `d_smile < -0.002` та `d_lip_raise >= -0.015` та ((`d_brow < -0.012`) або (`d_ear < -0.02` та `d_mar < 0.08` та `d_brow < -0.004`)) |
