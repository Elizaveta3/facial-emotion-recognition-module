...
Система розпізнавання емоцій у реальному часі, яка використовує точки обличчя MediaPipe Face Mesh та класифікацію на основі правил для визначення емоцій із потоку вебкамери. Система обчислює геометричні параметри обличчя (відкритість очей, форма рота, положення брів, кривизна усмішки, асиметрію рота та підняття верхньої губи) та класифікує їх в одну з восьми емоцій: **Happy** (Радість), **Fear** (Страх), **Surprised** (Здивування), **Angry** (Злість), **Sad** (Сум), **Disgusted** (Огида), **Contempt** (Презирство) або **Neutral** (Нейтральний).
...
Сім скалярних параметрів обчислюються з точок обличчя на кожному кадрі:
...
### Mouth Asymmetry — Асиметрія рота

Вимірює різницю по вертикалі між лівим та правим кутами рота, нормалізовану за висотою обличчя.

```
mouth_asymmetry = |left_corner_y - right_corner_y| / face_height
```

Використовується насамперед для `Contempt`: при однобічному піднятті кута рота значення зростає.

### Upper Lip Raise — Підняття верхньої губи

Вимірює вертикальну відстань між основою носа та верхньою губою, нормалізовану за висотою обличчя.

```
upper_lip_raise = |upper_lip_top_y - nose_base_y| / face_height
```

Менше значення означає, що верхня губа підтягується догори до носа. Це основний сигнал для `Disgusted`.
...
Очі також широко відкриті, але рот відкритий помірно, тобто слабше, ніж у `Surprised`.

| Режим | Умова |
|---|---|
| Абсолютний | `EAR > 0.30` та `0.15 < MAR <= 0.5` та `smile < 0.005` |
| Дельта | `d_ear > +0.04` та `0.08 < d_mar <= 0.35` та `d_smile < 0.006` |
...
### 4. Contempt (Презирство)

Пріоритетно визначається по асиметрії рота. Рот при цьому залишається відносно закритим, а сильне підняття верхньої губи не повинно бути домінуючим сигналом.

| Режим | Умова |
|---|---|
| Абсолютний | `mouth_asymmetry > 0.013` та `MAR < 0.15` та `upper_lip_raise >= 0.04` |
| Дельта | `d_asym > 0.008` та `d_mar < 0.08` та `d_lip_raise > -0.015` |

### 5. Disgusted (Огида)

Пріоритетно визначається по підняттю верхньої губи. Для цієї емоції також потрібні вузькі очі та мала відстань від очей до брів. Рот має бути злегка привідкритий, а не повністю закритий.

| Режим | Умова |
|---|---|
| Абсолютний | `EAR < 0.26` та `brow_dist < 0.118` та `upper_lip_raise < 0.04` та `0.08 < MAR < 0.15` |
| Дельта | `d_ear < -0.03` та `d_brow < -0.012` та `d_lip_raise < -0.02` та `0.03 < d_mar < 0.08` |

### 6. Angry (Злість)

Визначається по насуплених бровах та очах нейтральної ширини, але тільки коли рот залишається закритим. Сильне підняття верхньої губи блокує цю категорію, щоб `Angry` не поглинав `Disgusted`.

| Режим | Умова |
|---|---|
| Абсолютний | `smile < 0.005` та `upper_lip_raise >= 0.05` та `MAR < 0.08` та `brow_dist < 0.118` та `0.26 <= EAR <= 0.30` |
| Дельта | `d_smile < -0.002` та `d_lip_raise >= -0.015` та `d_mar < 0.03` та `d_brow < -0.012` та `-0.03 <= d_ear <= +0.04` |

### 7. Sad (Сум)

Кути рота опущені, а брови залишаються розслабленими або трохи піднятими. У calibrated-режимі правило м'якше і шукає падіння відносно вашого власного нейтрального виразу.

| Режим | Умова |
|---|---|
| Абсолютний | `smile < -0.012` та `EAR >= 0.245` та `brow_dist >= 0.128` та `mouth_asymmetry < 0.009` |
| Дельта | `d_smile < -0.003` та `d_ear >= -0.03` та `d_brow >= -0.004` та `d_asym < 0.008` |

### 8. Neutral (Нейтральний)
